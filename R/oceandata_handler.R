oceandata_sync=function(dataset) {
    ...
    ## expect that dataset$method_flags will contain the search and dtype components of the post string (i.e. search=...&dtype=... in "dtype=L3m&addurl=1&results_as_file=1&search=A2002*DAY_CHL_chlor*9km*")
    ...
    for (this_url in myfiles) {
        ...
        dummy$method_flags=paste(dummy$method_flags,"-O",oceandata_url_mapper(this_url)," ")
...
oceandata_url_mapper=function(this_url,path_only=FALSE,sep=.Platform$file.sep) {
    ...
    ## Mapped files (L3m) should become oceandata.sci.gsfc.nasa.gov/platform/Mapped/timeperiod/spatial/parm/[year]
    ## [year] only for 8Day,Daily,Rolling_32_Day,
    ...
    url_parts=str_match(this_url,".*getfile/([ASTC])([[:digit:]]+)\\.(L3m|L3b)_([[:upper:][:digit:]]+)_(.*?)_(9|4)(km)?\\.bz2")
    ...
    this_parm_folder=sapply(parm_str,function(z){ grepl(paste0("^",z,"$"),url_parts$parm) })
    this_parm_folder=parm[which(this_parm_folder)]
    switch(url_parts$type,
           L3m={ out<-paste("oceandata.sci.gsfc.nasa.gov",platform_map[[url_parts$platform]],"Mapped",timeperiod_map[[url_parts$timeperiod]],paste0(url_parts$spatial,"km"),this_parm_folder,sep=sep)
                 if (url_parts$timeperiod %in% c("8D","DAY","R32")) {
                     out<-paste(out,substr(url_parts$date,1,4),sep=sep)
                 }
                 if (!path_only) {
